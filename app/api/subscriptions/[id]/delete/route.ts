...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  await keys.updateOne(
    { _id: parsed.data.id },
    {
      $set: { disabled: true, deleted: true, deleted_at: now, deleted_by: session.sub, updated_at: now }
    }
  );

  audit('subscription.deleted', { user: session.sub, subscription: parsed.data.id });
